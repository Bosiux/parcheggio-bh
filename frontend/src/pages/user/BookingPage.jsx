...
import {
  Card, CardBody, Button, Chip, Input, Select, SelectItem, Spinner,
...
export default function BookingPage() {
  ...
  const handleConfirmBooking = async (onClose) => {
    ...
    try {
      const result = await bookArea(areaId, { startDate, startHour, durationHours });
      setBooking(result); setSuccess(true); onClose();
    } catch (err) {
      setError(err.message || "Errore durante la prenotazione."); onClose();
    } finally {
      setLoading(false);
    }
  };
      ...
      <Layout>
        <div style={{ maxWidth: 500, margin: "3rem auto", textAlign: "center" }}>
          <div style={{
            ...
            fontSize: 48, margin: "0 auto 1.5rem", color: "white", fontWeight: 700,
  ...
  return (
    <Layout>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {/* Area info card */}
        <Card shadow="none" classNames={{ base: `${CARD_BASE} !border-yellow-500/20 mb-6` }}>
          <CardBody style={{ padding: "1.75rem" }}>
            {loadingAvail ? (
              <div style={{ display: "flex", justifyContent: "center", padding: "2rem" }}>
                <Spinner color="warning" label="Verifica disponibilità..." />
              </div>
            ) : (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem", padding: "1rem", background: "var(--surface-04)", borderRadius: 10, marginBottom: "1.5rem" }}>
                  {[
                    ...
                    { label: "Capienza",     value: total,                                                          color: "#94a3b8" },
                ...
                <div style={{ background: "rgba(189,178,60,0.06)", border: "1px solid rgba(189,178,60,0.15)", borderRadius: 10, padding: "1rem" }}>
                  ...
                  <div style={{ display: "grid", gap: "0.75rem", marginBottom: "0.9rem" }}>
                    <Input type="date" label="Data inizio" labelPlacement="outside" value={startDate} onValueChange={setStartDate} variant="bordered" />
                    <Input type="time" label="Ora inizio"  labelPlacement="outside" value={startHour}  onValueChange={setStartHour}  variant="bordered" />
                    <Select
                      label="Durata prenotazione"
                      labelPlacement="outside"
                      selectedKeys={new Set([durationHours])}
                      onSelectionChange={(keys) => { const v = Array.from(keys)[0]; if (v) setDurationHours(String(v)); }}
                      variant="bordered"
                    >
                      {[1,2,3,4,6,8,12,24].map((h) => <SelectItem key={String(h)}>{`${h} ore`}</SelectItem>)}
                    </Select>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    {[
                      { label: "💳 Tariffa",        value: isUnderMaintenance ? "Sospesa" : `€ ${hourlyRate.toFixed(2)}/h` },
                      { label: "⏱️ Durata",         value: `${durationHours} ore` },
